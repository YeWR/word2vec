...
def load_test_file(file_name, word2id):
    ...
    for i, line in enumerate(lines):
        ...
        word1_lst.append(word2id[data[0]])
        word2_lst.append(word2id[data[1]])
...
if __name__=='__main__':
    ...
    parser.add_argument('--load_path', type=str, default='res/lr=0.0025-embed=300-vocab=10000/model.pth',
                        help='load_path')
    ...
    with torch.no_grad():
        ...
        word1_id = torch.LongTensor(word1_lst)
        word2_id = torch.LongTensor(word2_lst)

        word1_embed = model.inference(word1_id)
        word2_embed = model.inference(word2_id)

        similarity = torch.cosine_similarity(word1_embed, word2_embed, dim=0)
